...
import { useRoom, useMyPresence, useOthers, useStorage, useStorageStatus, useSelf } from "@/liveblocks.config"
...
export default function Editor({ theme }: EditorProps) {
  ...
  useEffect(() => {
    if (!editorReady || !editorRef.current || !isStorageLoaded || !myPresence.currentFile) return
  ...
  useEffect(() => {
    if (!editorReady || !editorRef.current || !files || !myPresence.currentFile) return
  ...
  useEffect(() => {
    ...
    others.forEach((user) => {
      if (!user.presence.cursor || !user.presence.username || user.presence.currentFile !== myPresence.currentFile)
        return
      ...
      if (selection) {
        ...
        const decorationIds = editor.deltaDecorations([], [selectionDecoration])
