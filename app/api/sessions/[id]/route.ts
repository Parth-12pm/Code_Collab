...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const editorSession = await EditorSession.findOne({ roomId: id })
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (editorSession.createdBy !== session.user.id) {
      return NextResponse.json({ error: "Only the owner can delete a session" }, { status: 403 })
    }

    await EditorSession.deleteOne({ roomId: id })
